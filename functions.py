...
def get_most_fuel_efficient_cars(n=5):
    #returns the top n rows sorted by miles per gallon (mpg) in descending order because higher mpg means lower fuel consumption
    return df.sort_values("mpg", ascending=False).head(n) 



def get_origin_statistics(): 
    #returns summary statistics about the cars grouped by their origin, including the count of cars, average miles per gallon (mpg), average horsepower, and average weight for each origin category
